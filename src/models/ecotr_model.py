...
class EOTR(nn.Module):
    def __init__(self, backbone, coarse_tsfm, mid_tsfm, fine_tsfm, args):
        ...
        self.coarse_input_proj = nn.Sequential(
            nn.Conv2d(
                backbone.backbone.body.config['dense'][-1][-1], self.hidden_dims[-1],
                kernel_size=(1, 1), stride=(1, 1), bias=False), nn.GroupNorm(16, self.hidden_dims[-1]),
        )
        ...
        self.mid_input_proj = nn.Sequential(
            nn.Conv2d(
                backbone.backbone.body.config['dense'][-1][-2], self.hidden_dims[-2],
                kernel_size=(1, 1), stride=(1, 1), bias=False), nn.GroupNorm(16, self.hidden_dims[-2]),
        )
        ...
        self.fine_input_proj = nn.Sequential(
            nn.Conv2d(
                ...
                self.hidden_dims[0], kernel_size=(1, 1), stride=(1, 1), bias=False),
    ...
    def mid_ecotr(self, mid_feature, queries, corrs, out):  # val: samples.tensors
        ...
        mid_queries = denormalize_kpts(mid_query_corrs_pairs[..., :2], 'l')
        mid_corrs_c = denormalize_kpts(mid_query_corrs_pairs[..., 2:], 'r')
        ...
        if 'alone_chosen_index' in out_dict.keys():
            ...
            if patch_idx[..., 0].shape[0] != 0 and alone_idx[..., 0].shape[0] != 0:
                all_rerank_idx = torch.cat([patch_idx[..., 0], alone_idx[..., 0].expand(-1, patch_idx.shape[1])], dim=0)
...
def build(args):
    ...
    if args.transformer_type == 'fully':
        coarse_transformer = build_transformer(args, backbone.backbone.body.config['dense'][-1][-1],
                                               backbone.backbone.body.config['dense'][-1][-1] * 2)
        mid_transformer = build_transformer(args, backbone.backbone.body.config['dense'][-1][-2],
                                            backbone.backbone.body.config['dense'][-1][-2] * 2)
        fine_transformer = build_transformer(args, backbone.backbone.body.config['dense'][-1][0],
