...
def generate_fine_patch_inference_kmeans(
    ...
    corrs_centroids = torch.sum(patch_corrs, dim=2) / (torch.sum((torch.sum(patch_corrs, dim=-1) > 0).float(),
                                                                 dim=2, keepdim=True) + EPS)  # [b, k, 2]
    ...
    patch_queries = (patch_queries - patch_queries_centroids.unsqueeze(2).expand_as(patch_queries)) / (
            window_size / h) + 0.5
    ...
    queries_features_unfold = F.unfold(queries_features, kernel_size=window_size, stride=1,
                                       padding=window_size // 2)  # [b, (cww) l]
    queries_features_unfold = queries_features_unfold.transpose(1, 2)  # [b, l, (cww)]
    patch_queries_centroids_idx_cww = patch_queries_centroids_idx.unsqueeze(1).expand(-1,
                                                                                      queries_features_unfold.size(-1),
                                                                                      -1).transpose(1,
                                                                                                    2)  # [b, k (cww)]
    patch_queries_features = torch.gather(queries_features_unfold, dim=1,
                                          index=patch_queries_centroids_idx_cww)  # [b, k, (cww)]
    patch_queries_features = rearrange(patch_queries_features, 'n l (c ww) -> n l c ww',
                                       ww=window_size ** 2)  # [b, k, c, ww]
    patch_queries_features = patch_queries_features.reshape(b, k, c, window_size, window_size)  # [b, k, c, w, w]

    corrs_features_unfold = F.unfold(corrs_features, kernel_size=window_size, stride=1,
                                     padding=window_size // 2)  # [b, (cww) l]
    corrs_features_unfold = corrs_features_unfold.transpose(1, 2)  # [b, l, (cww)]
    patch_corrs_centroids_idx_cww = patch_corrs_centroids_idx.unsqueeze(1).expand(-1, corrs_features_unfold.size(-1),
                                                                                  -1).transpose(1, 2)  # [b, k (cww)]
    patch_corrs_features = torch.gather(corrs_features_unfold, dim=1,
                                        index=patch_corrs_centroids_idx_cww)  # [b, k, (cww)]
    patch_corrs_features = rearrange(patch_corrs_features, 'n l (c ww) -> n l c ww',
                                     ww=window_size ** 2)  # [b, k, c, ww]
    ...
    patch_queries_features = rearrange(patch_queries_features, 'b k c w1 w2 -> (b k) c w1 w2')[
        patch_nonzero_idxs]  # [s, c, w, w]
    patch_corrs_features = rearrange(patch_corrs_features, 'b k c w1 w2 -> (b k) c w1 w2')[
        patch_nonzero_idxs]  # [s, c, w, w]
    ...
    alone_queries = (torch.gather(queries, dim=1, index=alone_map_i) * alone_map_v).unsqueeze(
        2)  # Tensor [b, max_alone_q, 1, 2]
    alone_corrs = (torch.gather(corrs, dim=1, index=alone_map_i) * alone_map_v).unsqueeze(
        2)  # Tensor [b, max_alone_q, 1, 2]
    ...
    alone_corrs_centroids_idx = alone_corrs_centroids_idx[..., 1] * w + alone_corrs_centroids_idx[
        ..., 0]  # [b, max_alone_q]

    alone_queries_centroids_idx_cww = alone_queries_centroids_idx.unsqueeze(1).\
        expand(-1, queries_features_unfold.size(-1), -1).transpose(1, 2)
    # [b, max_alone_q (cww)]
    alone_queries_features = torch.gather(queries_features_unfold, dim=1,
                                          index=alone_queries_centroids_idx_cww)  # [b, max_alone_q, (cww)]
    alone_queries_features = rearrange(alone_queries_features, 'n l (c ww) -> n l c ww',
                                       ww=window_size ** 2)  # [b, max_alone_q, c, ww]
    alone_queries_features = alone_queries_features.reshape(b, max_alone_q, c, window_size,
                                                            window_size)  # [b, max_alone_q, c, w, w]

    # [b, max_alone_q (cww)]
    alone_corrs_centroids_idx_cww = alone_corrs_centroids_idx.unsqueeze(1).expand(-1, corrs_features_unfold.size(-1),
                                                                                  -1).transpose(1, 2)

    alone_corrs_features = torch.gather(corrs_features_unfold, dim=1,
                                        index=alone_corrs_centroids_idx_cww)  # [b, max_alone_q, (cww)]
    alone_corrs_features = rearrange(alone_corrs_features, 'n l (c ww) -> n l c ww',
                                     ww=window_size ** 2)  # [b, max_alone_q, c, ww]
    alone_corrs_features = alone_corrs_features.reshape(b, max_alone_q, c, window_size,
                                                        window_size)  # [b, max_alone_q, c, w, w]
    ...
    alone_queries_features = rearrange(alone_queries_features, 'b k c w1 w2 -> (b k) c w1 w2')[
        alone_nonzero_idxs]  # [s, c, w, w]
    alone_corrs_features = rearrange(alone_corrs_features, 'b k c w1 w2 -> (b k) c w1 w2')[
        alone_nonzero_idxs]  # [s, c, w, w]
