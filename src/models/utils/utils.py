...
def draw_matches(img_a, img_b, corrs, type='dot'):
    ...
    if type == 'dot':
        for i in range(len(corrs)):
            ...
            cv2.circle(canvas, (int(kp_a[i, 0]), int(kp_a[i, 1])), 2, value_2_jet(value), 2)
            cv2.circle(canvas, (int(kp_b[i, 0]), int(kp_b[i, 1])), 2, value_2_jet(value), 2)
            # cv2.circle(canvas, (int(kp_a[i, 0]), int(kp_a[i, 1])), 4, (0, 128, 255), -1)
            # cv2.circle(canvas, (int(kp_b[i, 0]), int(kp_b[i, 1])), 4, (0, 128, 255), -1)
...
# def draw_matches(img_a, img_b, corrs, type='dot'):
# def draw_matches(img_a, img_b, kp_a, kp_b, corrs, type='dot'):
#     h0, w0 = img_a.shape[:2]
#     h1, w1 = img_b.shape[:2]
#     maxh, _ = max(h0, h1), max(w0, w1)
#     canvas = np.zeros((maxh, w0 + w1, 3)).astype(np.uint8)
