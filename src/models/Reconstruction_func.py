...
def optical_flow_from_a_to_b(cap_a, cap_b):
    ...
    optical_flow = PointCloudProjector.pcd_2d_to_img_2d_np(
        PointCloudProjector.pcd_3d_to_pcd_2d_np(pcd_from_cap_b, cap_a_intrinsic, cap_a.cam_pose.world_to_camera[0:3, :],
                                                cap_a_img_size, keep_z=True, crop=True, filter_neg=True,
                                                norm_coord=False), cap_a_img_size, has_z=True, keep_z=False)
    return optical_flow
...
class PointCloudProjector:
    ...
    @staticmethod
    def pcd_3d_to_pcd_2d_np(pcd, intrinsic, extrinsic, size, keep_z: bool, crop: bool = True, filter_neg: bool = True,
                            norm_coord: bool = True, return_index: bool = False):
        assert isinstance(pcd, np.ndarray), 'cannot process data type: {0}'.format(type(pcd))
        assert isinstance(intrinsic, np.ndarray), 'cannot process data type: {0}'.format(type(intrinsic))
        assert isinstance(extrinsic, np.ndarray), 'cannot process data type: {0}'.format(type(extrinsic))
        assert len(pcd.shape) == 2 and pcd.shape[
            1] >= 3, 'seems the input pcd is not a valid 3d point cloud: {0}'.format(pcd.shape)
        ...
        if crop:
            valid_mask_2 = (image_points[:, 0] >= 0) * (image_points[:, 0] < size[1] - 1) * (
                        image_points[:, 1] >= 0) * (image_points[:, 1] < size[0] - 1)
        else:
        ...
        if keep_z:
            image_points = np.concatenate([image_points[valid_mask_2], camera_points[valid_mask_2][:, 2:3],
                                           pcd[valid_mask_1][:, 3:][valid_mask_2]], axis=1)
        else:
    ...
    def pcd_2d_to_img_2d_np(pcd, size, has_z=False, keep_z=False):
        assert len(pcd.shape) == 2 and pcd.shape[-1] >= 2, 'seems the input pcd is not a valid point cloud: {0}'.format(
            pcd.shape)
...
def triangulate_rays_to_pcd(center_a, dir_a, center_b, dir_b):
    ...
    c = B - A
    D = A + a * ((-np.sum(a * b, axis=1) * np.sum(b * c, axis=1) + np.sum(a * c, axis=1) * np.sum(b * b, axis=1)) / (
                np.sum(a * a, axis=1) * np.sum(b * b, axis=1) - np.sum(a * b, axis=1) * np.sum(a * b, axis=1)))[
        ..., None]
    return D
